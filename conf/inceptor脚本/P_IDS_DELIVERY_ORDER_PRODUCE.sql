  ...
  BEGIN
    ...
    l_sqlBuf := "SELECT  T.LSH,T.TZZH_ID,T.SEQNO,T.WTH,T.KHH,T.KHXM,T.GDH,T.JYS,T.BZ,T.YYB,T.ZQDM,T.ZQMC,T.ZQLB,T.JYLB," ||
                ...
                "  FROM " || F_IDS_GET_TABLENAME('sparkJgmxlsHis', I_KHH) ||"  T
				  ...
				  LEFT JOIN dsc_CFG.INFO_TZQDMBG BG ON (BG.BGLB = 3 AND T.CJRQ = BG.BGRQ AND T.ZQDM = BG.BGDM AND T.JYS = BG.JYS)       -20210121
				  LEFT JOIN dsc_CFG.INFO_TZQDMBG BGQ ON (BGQ.BGLB = 3 AND T.CJRQ = BGQ.BGRQ AND T.ZQDM = BGQ.ZQDM AND T.JYS = BGQ.JYS)
                  LEFT JOIN (SELECT
                    DISTINCT ZQDM
                              , GPDM,sgrq,
 ...
 --修正后的持仓成本,顺带更新盲拆的清算序列号
  BEGIN
    ...
    l_sqlBuf := "SELECT T.TZZH_ID,
                       ...
                       T.CJJE,
                       T.LXJE,
                       CASE WHEN HG.KHH IS NOT NULL THEN 0-HG.CCCB ELSE T.YSJE END AS YSJE,
                       T.JSRQ,
                       T.BCZQYE,
                       T.YSSL,
                       T.ZXJ,
