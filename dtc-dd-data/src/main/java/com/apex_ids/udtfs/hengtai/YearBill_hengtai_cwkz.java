...
import com.apex_ids.util.MyEnum.YearBillCwkzIn;
 ...
 */
public class YearBill_hengtai_cwkz extends GenericUDTF {
    private static List<String> result=null;
    private static final Logger log = LoggerFactory.getLogger(YearBill_hengtai_cwkz.class);
    ...
    private List<Object> caculate(List<List<Object>> bills) {
        String khh=bills.get(0).get(YearBillCwkzIn.khh.getIndex())+"";
        int ksrq = (int)bills.get(0).get(YearBillCwkzIn.rq.getIndex());
        ...
        int jsrq = (int)bills.get(bills.size()-1).get(YearBillCwkzIn.rq.getIndex());

        for (List<Object> bill: bills){
            for(String m:result){
                if(bill.get(YearBillCwkzIn.rq.getIndex()).toString().equals(m.split(":")[0])){
                    khData.add(Double.parseDouble(bill.get(YearBillCwkzIn.zqsz_jzjy.getIndex())+"")
                            +Double.parseDouble(bill.get(YearBillCwkzIn.zqsz_rzrq.getIndex())+""));
        ...
        double score =0.0d;
        if(flag){
            score =getPearsonCorrelationScore(khData, hs300Data);
        }
        ...
        Collections.sort(bills,
                new Comparator<List<Object>>() {
                    ...
                    public int compare(List<Object> d1, List<Object> d2) {
                        Integer d1Str = Integer.valueOf(d1.get(YearBillCwkzIn.rq.getIndex()).toString());
                        Integer d2Str = Integer.valueOf(d2.get(YearBillCwkzIn.rq.getIndex()).toString());
    ...
    private List<Object> tranDataType(List<String> bill){
        ...
        for (int i=0; i<bill.size(); i++){
            result.add(UdfUtil.parseObject(bill.get(i), YearBillCwkzIn.values()[i].getType()));
        }
        return result;
    }
    ...
    private static List<String>getYearHS300(int ksrq,int jsrq){
        ...
        try {
            props.load(YearBill_hengtai_cwkz.class.getResourceAsStream("/jdbc.properties"));
