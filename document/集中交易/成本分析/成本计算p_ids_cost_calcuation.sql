...
--------------------------------------    //1、获取货币基金申购赎回--------------------------------------
  BEGIN
    ...
    l_sqlBuf := "SELECT T.TZZH_ID," ||
                ...
                "                 D.SGSHDM," ||
                "                 JG.BZ," ||
                "                 CASE WHEN JYLB='29' AND YSJE<0 AND CJRQ<JSRQ THEN JSRQ ELSE CJRQ END AS JSRQ," ||--交易类别29:ETF申购
                "                 SUM(CASE" ||
                "                       WHEN (JYLB = '30' AND YSJE > 0) OR (JYLB = '29' AND YSJE > 0) THEN" ||  --etf赎回
                "                        ABS(YSJE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                     END) AS DRSHJE," ||  --当日赎回金额
                "                 SUM(CASE" ||
                "                       WHEN JYLB = '30' AND YSSL < 0 AND SEQNO!=0 THEN" ||
                "                        ABS(YSSL)" ||
                "                       ELSE" ||
                "                        0" ||
                "                     END) AS DRSHSL," ||  --当日赎回数量
                "                 SUM(CASE" ||
                "                       WHEN JYLB = '29' AND YSJE < 0 THEN" ||
                "                        ABS(YSJE)" ||
                "                       ELSE" ||
                "                        0" ||
                "                     END) AS DRSGJE," ||  --当日申购金额
                "                 SUM(CASE" ||
                "                       WHEN JYLB = '29' AND YSSL > 0 THEN" ||
                "                        ABS(YSSL)" ||
                "                       ELSE" ||
                "                        0" ||
                "                     END) AS DRSGSL" ||  --当日申购数量
                "            FROM " || F_IDS_GET_TABLENAME('sparkJgmxlsHis', I_KHH)  ||" JG, CUST.T_HBJJXX D" ||--货币基金
                "           WHERE JG.JYS = D.JYS" ||
...
----------------------   //更新到表--------------------------------------
  ...
  BEGIN
    ...
    l_sqlBuf := "select * from CUST.T_ZQYE_CBJS_HBJJSGSH where DZRQ_SG = " || l_sqlWhereCurrentDay; --申购交收日期
  ...
  BEGIN
    ...
    l_sqlBuf := "select * from CUST.T_ZQYE_CBJS_HBJJSGSH where WTRQ_SH = " || l_sqlWhereCurrentDay;  --赎回交收日期
